...
#[test]
fn unroll_extension_file_doesnt_exist() -> TestResult {
    let template_dir = assert_fs::TempDir::new()?;
    let unroll_dir = assert_fs::TempDir::new()?;
...
#[test]
fn unroll_extension_file_exists() -> TestResult {
    let template_dir = assert_fs::TempDir::new()?;
    let unroll_dir = assert_fs::TempDir::new()?;
...
#[test]
fn unroll_extension_file_dont_duplicate() -> TestResult {
    let template_dir = assert_fs::TempDir::new()?;
    let unroll_dir = assert_fs::TempDir::new()?;
...
"#,
    ...
    cmd.arg("make")
        ...
        .assert()
        .success()
        .stderr(predicates::str::contains("WARN: file_TEST.txt already contains identical content"));
